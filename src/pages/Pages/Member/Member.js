...
const Member = () => {
	...
	useEffect(() => {
		...
		if (memberDetail.member?.loginWith) {
			setLoginWith({ value: memberDetail.member?.loginWith, label: memberDetail.member?.loginWith === 1 ? "Phone Number" : memberDetail.member?.loginWith === 2 ? "Email" : memberDetail.member?.loginWith === 3 ? "Gmail" : null });
		}
				...
				<ModalBody>
					...
					>
						...
						<div className="mb-2">
							...
							<Select isClearable={true} value={loginWith} onChange={handleSelectLoginWith} options={optionLoginWith} isDisabled={memberDetail.member} />
						</div>
						<div className="mb-2">
