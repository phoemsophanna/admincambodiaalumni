...
import {
	...
	fetchGalleryDetail,
	refreshGalleryList,
	resetCreateGalleryFlag,
	resetGalleryShowDetail,
} from "../../../store/actions";
...
const Gallery = () => {
	document.title = "Gallery | Admin & Dashboards";
	const dispatch = useDispatch();
	const [UID, setUID] = useState(null);
	const [file, setFile] = useState([]);

	const createGallerySelector = createSelector(
		(state) => state.CreateGalleryReducer,
		(layout) => layout
	);
	const createGalleryDetailSelector = createSelector(
		(state) => state.GalleryDetailReducer,
		(layout) => layout
	);

	const useCreateGallerySelect = useSelector(createGallerySelector);
	const campaignCategoryDetail = useSelector(createGalleryDetailSelector);
	...
	function closeModal() {
		...
		setFile([]);
		dispatch(resetGalleryShowDetail());
	}

	const handleRefresh = () => {
		dispatch(refreshGalleryList());
	};

	const handleShowGalleryDetail = (campaignCategoryId) => {
		setmodal_backdrop(true);
		dispatch(fetchGalleryDetail(campaignCategoryId));
	};

	const handleDeleteGallery = () => {
		if (UID) {
			dispatch(deleteGallery(UID));
		}
	};

	useEffect(() => {
		if (useCreateGallerySelect.success && !useCreateGallerySelect.isLoading) {
			dispatch(resetCreateGalleryFlag());
			setmodal_backdrop(false);
			campaignCategoryValidation.resetForm();
			setFile([]);
			dispatch(resetGalleryShowDetail());
	...
	}, [dispatch, campaignCategoryValidation, setDeleteModal, useCreateGallerySelect, campaignCategoryDetail]);
	...
	return (
		<React.Fragment>
			<div className="page-content">
				<Container fluid>
					<BreadCrumb title="Gallery" pageTitle="Dashboard" />
					<Row>
						<Col lg={12}>
							<Card>
								<CardHeader>
					...
					<Row>
						<Col lg={12}>
							<TableList
								onShowDetail={handleShowGalleryDetail}
								onDeleteGallery={(id) => {
									setDeleteModal(true);
									setUID(id);
								}}
			...
			<DeleteModal
				...
				isLoading={useCreateGallerySelect.isLoading}
			/>
			<Modal
			...
			>
				<ModalHeader className="bg-light p-3 text-light" toggle={closeModal}>
					{campaignCategoryDetail.campaignCategory ? "Update Gallery" : "Create Gallery"}
				</ModalHeader>
				...
				<ModalBody>
					...
					>
									...
									<div className="col-md-12 col-12">
										...
										<div style={{ width: "100%" }}>
											<FilePond
												...
												maxFiles={1}
												name="file"
												server={`${api.BASE_URL}/save-image/gallery`}
						...
						<div className="text-end">
							...
							{useCreateGallerySelect.isLoading ? (
